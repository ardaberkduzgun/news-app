...
const Finance = () => {
  ...
  useEffect(() => {
    ...
    const newsApiUrl = `${process.env.REACT_APP_NEWSAPI_URL}/everything?q=finance&apiKey=${newsApiKey}`;

    const guardianApiKey = process.env.REACT_APP_THEGUARDIAN_KEY;
    const guardianApiUrl = `${process.env.REACT_APP_THEGUARDIAN_URL}?section=money&api-key=${guardianApiKey}`;

    const nyTimesApiKey = process.env.REACT_APP_NYT_KEY;
    const nyTimesApiUrl = `${process.env.REACT_APP_NYT_URL}/search/v2/articlesearch.json?q=financial&api-key=${nyTimesApiKey}`;
