...
const Travel = () => {
  ...
  useEffect(() => {
    ...
    const newsApiUrl = `${process.env.REACT_APP_NEWSAPI_URL}/everything?q=travel&apiKey=${newsApiKey}`;

    const guardianApiKey = process.env.REACT_APP_THEGUARDIAN_KEY;
    const guardianApiUrl = `${process.env.REACT_APP_THEGUARDIAN_URL}?section=travel&&api-key=${guardianApiKey}`;

    const nyTimesApiKey = process.env.REACT_APP_NYT_KEY;
    const nyTimesApiUrl = `${process.env.REACT_APP_NYT_URL}/topstories/v2/travel.json?api-key=${nyTimesApiKey}`;
