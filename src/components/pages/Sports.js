...
const Sports = () => {
  ...
  useEffect(() => {
    ...
    const newsApiUrl = `${process.env.REACT_APP_NEWSAPI_URL}/everything?q=sports&apiKey=${newsApiKey}`;

    const guardianApiKey = process.env.REACT_APP_THEGUARDIAN_KEY;
    const guardianApiUrl = `${process.env.REACT_APP_THEGUARDIAN_URL}?section=sport&api-key=${guardianApiKey}`;

    const nyTimesApiKey = process.env.REACT_APP_NYT_KEY;
    const nyTimesApiUrl = `${process.env.REACT_APP_NYT_URL}/topstories/v2/sports.json?api-key=${nyTimesApiKey}`;
